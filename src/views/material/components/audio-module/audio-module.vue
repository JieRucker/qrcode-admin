...
<template>
  <popup-modal :block="block" @on-close="onClose" @on-ok="saveAudioModulePopup">
    ...
    <div class="module-group clearfix">
      ...
      <div class="module-control-content">
        <div class="upload-list" v-for="(item,index) in initial.audioList" style="margin-bottom: 10px">
          <Input style="width: 200px" v-model="item.audioName"></Input>
          <Input style="width: 200px;margin-left: 5px" v-model="item.audioUrl"></Input>
        ...
        <Button @click="getUpload" v-if="initial.audioList && !initial.audioList.length">上传文件</Button>
      </div>
    </div>
...
<script>
    ...
    methods: {
      ...
      initUpload() {
        ...
        this.uploader = WebUploader.create({
          ...
          accept: {
            title: 'Images',
            extensions: 'ogg,wav,mp3,mid',
            mimeTypes: '.ogg,.wav,.mp3,.mid'
          },
          auto: true,
          compress: false, //不压缩
          chunked: true, //分块传输
        ...
        this.uploader.on('fileQueued', (_file) => {
          ...
          return this.uploader.makeThumb(_file, (err, src) => {
            this.initial.audioList = new Array({
              audioName: _file.name,
              bucketName: '',
              ossKey: '',
              audioUrl: src,
       ...
       */
      handleRemove(index) {
        this.initial.audioList.splice(index, 1)
      },
      /**
       * 获取uploadId
       */
      ...
      saveAudioModulePopup() {
        ...
        this.$api.qrcodeInterface.saveModuleAudioVO(param).then(res => {
          if (res.data.success) {
  ...
  class ToolFunctor {
    ...
    _get(res) {
      ...
      target.audioList = (() => {
        let a = [];
        if (res.moduleObject && res.moduleObject.audioName) {
          a = new Array({
            audioName: res.moduleObject.audioName,
            audioUrl: res.moduleObject.audioUrl,
    ...
    _set() {
      let target = new Object(null);
      let audioList = this.tis.initial.audioList;
      ...
      target.moduleVOJson = (() => {
        let options = {
          ...
          moduleObject: {
            ...
            audioName: audioList.length ? audioList[0]['audioName'] : '',
            audioUrl: audioList.length ? (this.tis.$Global.isUrl(audioList[0]['audioUrl']) ? audioList[0]['audioUrl'] : '') : '',
            bucketName: audioList.length ? audioList[0]['bucketName'] : '',
            ossKey: audioList.length ? audioList[0]['ossKey'] : '',
            fileSize: audioList.length ? audioList[0]['fileSize'] : '',
            id: audioList.length ? audioList[0]['id'] : ''
          }
        };
