...
<template>
  <popup-modal :block="block" @on-close="onClose" @on-ok="saveFileModulePopup">
    ...
    <div class="module-group clearfix">
      ...
      <div class="module-control-content">
        <div class="upload-list" v-for="(item,index) in initial.fileList" style="margin-bottom: 10px">
          <Input style="width: 200px" v-model="item.fileName"></Input>
          <Input style="width: 200px;margin-left: 5px" v-model="item.fileUrl"></Input>
    ...
    methods: {
      ...
      initUpload() {
        ...
        this.uploader = WebUploader.create({
          ...
          accept: {
            title: 'Images',
            extensions: 'png,jpg,jpeg,gif,bmp,png,flv,swf,mkv,avi,rmvb,mpeg,mpg,ogg,ogv,mov,wmv,mp4,webm,mp3,wav,mid,rar,zip,tar,gz,7z,bz2,cab,iso,doc,docx,xls,xlsx,ppt,pptx,pdf,txt,md,xml',
            mimeTypes: '.png,.jpg,.jpeg,.gif,.bmp,.png,.flv,.swf,.mkv,.avi,.rmvb,.mpeg,.mpg,.ogg,.ogv,.mov,.wmv,.mp4,.webm,.mp3,.wav,.mid,.rar,.zip,.tar,.gz,.7z,.bz2,.cab,.iso,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,.txt,.md,.xml'
          },
          auto: true,
          compress: false, //不压缩
          chunked: true, //分块传输
        ...
        this.uploader.on('fileQueued', (_file) => {
          ...
          return this.uploader.makeThumb(_file, (err, src) => {
            this.initial.fileList = new Array({
              fileName: _file.name,
              bucketName: '',
              ossKey: '',
              fileUrl: src,
       ...
       */
      handleRemove(index) {
        this.initial.fileList.splice(index, 1)
      },
      /**
       * 获取uploadId
       */
      ...
      saveFileModulePopup() {
        ...
        this.$api.qrcodeInterface.saveModuleFileVO(param).then(res => {
          if (res.data.success) {
  ...
  class ToolFunctor {
    ...
    _get(res) {
      ...
      target.fileList = (() => {
        let a = [];
        if (res.moduleObject) {
          res.moduleObject.fileList.forEach(m => {
            a.push({
              fileName: m.fileName,
              fileUrl: m.fileUrl,
      ...
      target.moduleVOJson = (() => {
        let options = {
          ...
          moduleObject: {
            ...
            fileList: (() => {
              let a = [];
              [...this.tis.initial.fileList].forEach(m => {
                a.push({
                  fileName: m.fileName,
                  bucketName: m.bucketName,
                  ossKey: m.ossKey,
                  fileUrl: this.tis.$Global.isUrl(m.fileUrl) ? m.fileUrl : '',
